...
令 `dp[j]` 表示目前已使用 `s1` 前綴與 `s2` 前綴 `j` 個字元時，是否能組成 `s3` 前綴。當外層處理 `s1` 的前 `i` 個字元時，這個前綴的長度就是 `i + j`。

處理 `s3[i + j - 1]` 時，最後一個字元只有兩種來源：

- 取自 `s1`：前一個狀態是 `dp[j]`，因為尚未更新的 `dp[j]` 仍代表上方狀態，也就是只使用 `s1` 前 `i - 1` 個字元。
- 取自 `s2`：前一個狀態是 `dp[j - 1]`，因為它已在目前這一列更新，代表左方狀態，也就是使用 `s2` 前 `j - 1` 個字元。

只要其中一條路徑可行，`dp[j]` 就能設為 `true`。先檢查三個字串的總長度是否相等，可以立即排除不可能的情況；接著以一維陣列由左至右更新，符合 Follow up 要求的 `O(s2.length)` 額外空間。
    ...
    public boolean isInterleave(String s1, String s2, String s3) {
        ...
        for (int firstIndex = 1; firstIndex <= firstLength; firstIndex++) {
            ...
            for (int secondIndex = 1; secondIndex <= secondLength; secondIndex++) {
                ...
                // 更新前 dp[secondIndex] 是上方狀態，更新後 dp[secondIndex - 1] 是左方狀態。
                boolean takeFromFirst = dp[secondIndex]
                        && s1.charAt(firstIndex - 1) == target;
                boolean takeFromSecond = dp[secondIndex - 1]
                        && s2.charAt(secondIndex - 1) == target;
                dp[secondIndex] = takeFromFirst || takeFromSecond;
            }
        }
...
- `dp[0]` 代表只使用 `s1` 的前綴；第一列則代表只使用 `s2` 的前綴。這兩個邊界會逐字檢查，遇到不相等的字元後就會維持 `false`。
- 每個一般狀態只從「上方取 `s1`」或「左方取 `s2`」轉移，且最後要加入的字元必須與 `s3` 對應位置相同，因此不會漏掉任何合法交錯方式。
...
- 空間複雜度為 `O(n)`，其中 `n` 是 `s2.length()`；使用長度為 `n + 1` 的一維 DP 陣列。

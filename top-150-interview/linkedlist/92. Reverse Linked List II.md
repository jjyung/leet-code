...
同樣先找到反轉區段前一個節點 `beforeReverse`，以及反轉區段原本的第一個節點 `start`。接著使用 `prev`、`current`、`next` 三個指標，將整個區段像一般單向鏈結串列反轉一樣逐節點反轉。
...
完成反轉後，`prev` 是新的區段開頭，`current` 是區段後方的節點，而 `start` 已經成為新的區段尾端，最後將三段鏈結重新接回去。
...
class Solution {
    public ListNode reverseBetween(ListNode head, int left, int right) {
        ...
        // start 是原本區段的第一個節點，反轉後會成為區段尾端。
        ListNode start = beforeReverse.next;
        ListNode prev = null;
